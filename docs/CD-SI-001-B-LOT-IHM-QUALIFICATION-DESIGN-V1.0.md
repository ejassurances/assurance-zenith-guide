...
Une nouvelle soumission de la même intention est un **no-op idempotent**
(`deja_valide`), jamais un écrasement.
...
| Fichier | Rôle |
|---|---|
| `src/lib/email-context-validation.ts` | Moteur **pur**, sans I/O : application d'une intention humaine, contrôles de non-dégradation, idempotence, clé déterministe |
...
/** Cle deterministe d'idempotence (aucun champ JSON ajoute, usage memoire seul). */
...
- `sentinellesHumaines(observe).presente` et intention non strictement identique
  → `deja_valide` (aucune écriture) ; intention dégradante → `degradation_interdite` ;
...
6. `lignesAffectees === 0` → `conflit_concurrent`. `erreur` non nulle →
   `erreur_base`. Dans les deux cas : aucun retry, aucun second `UPDATE`.
...
- `0 ligne` → `conflit_concurrent` : l'IHM recharge l'état et exige une nouvelle
  décision explicite. Aucune retentative automatique.
- Erreur BDD → `erreur_base`, aucun retry.
- Aucun paramètre `force` n'existe dans ce lot : rien ne permet de franchir une
  sentinelle.
- Idempotence : `cleValidation` sur `emailId` + statut cible + `validated_by` +
  `validated_at` + corrections normalisées ; intention déjà appliquée →
  `deja_valide` sans écriture.
...
| IHM-15 | Double soumission identique | une seule écriture |
...
un GO FERME DG explicite sur le présent DESIGN V1.0.
