# CD-SI-001-B — LOT 4 — DESIGN V1.2

**MODÈLE DE PREUVES, CONVERGENCE ET ÉCRITURE SÉCURISÉE DES FK**

Version : V1.2

Statut : DESIGN CORRIGÉ — SOUMIS À AUDIT DG FINAL

Motif : corrections post-audit DG V1.1 (1 majeur — protection des sentinelles humaines ; 5 mineurs — identifiants de preuves, preuves Lot 2, concurrence, T-24, clé de déduplication)
...
| Décision DG post-audit V1.1 | Arbitrages obligatoires | Protection des sentinelles humaines, conventions de preuves, concurrence harmonisée, T-24, clé de déduplication |
...
Le JSON Schema 1.1.0 ne comporte aucun champ dédié `niveau`. La V1.2 n'en ajoute pas.
...
#### C.3.1 Convention d'identifiants des preuves Lot 3

Les preuves produites par le Lot 3 utilisent actuellement la convention d'identifiant :
...
Cette convention peut être utilisée par l'évaluateur comme **indice auxiliaire** pour déterminer le niveau porteur des preuves Lot 3 lorsque nécessaire. Elle **ne remplace jamais** la vérification des champs réels définie en C.3 : en cas de divergence ou d'identifiant non conforme, la décision est `REFUSER(niveau_porteur_indetermine)`.

#### C.3.2 Distinction des preuves Lot 2 et Lot 3

Le `ai_context` peut contenir deux familles de preuves :

- **preuves héritées du Lot 2** (extraction Gemini, statut `DETECTED`) : elles sont **conservées** à des fins contextuelles mais ne portent **aucun niveau doctrinal explicite**. Elles ne doivent **JAMAIS** être utilisées par le Lot 4 pour déduire un niveau N1–N9 si elles ne portent pas explicitement ce niveau ;
- **preuves produites par le Lot 3** (résolution déterministe, convention `L3-<niveau>-<n>`) : seules ces preuves, **dont le niveau est déterminable de manière sûre**, peuvent servir à déterminer le niveau porteur d'une écriture.

Toute preuve dont l'origine ou le niveau est indéterminé est traitée comme une preuve Lot 2 : descriptive, jamais porteuse.

---

## D. Matrice des preuves (doctrine N1–N9)
...
**K.5 Clé de déduplication normalisée de `modifications_apportees[]`.**

`modifications_apportees` est un `string[]` dans le schéma 1.1.0. La clé de déduplication est une **représentation textuelle normative et déterministe** à 6 composantes, concaténées dans cet ordre exact avec le séparateur `|` :

```text
source | champ | ancienne_valeur | nouvelle_valeur | niveau_ou_mode | preuve_ids
```

Règles de normalisation obligatoires :

| # | Règle |
|---|---|
| 1 | `source` : valeur exacte parmi les valeurs autorisées (`humain`, `regle_deterministe`, etc.) ; jamais vide |
| 2 | `champ` : nom exact de la FK ou du champ concerné, en minuscules (ex. `client_id`) |
| 3 | `ancienne_valeur` / `nouvelle_valeur` : valeur littérale, ou la chaîne exacte `null` pour une absence de valeur ; aucun espace superflu |
| 4 | `niveau_ou_mode` : niveau doctrinal (ex. `N3`) ou mode (ex. `rebond`, `humain`) ; jamais vide |
| 5 | `preuve_ids` : identifiants triés par ordre lexicographique, joints par `,` ; la chaîne exacte `-` si aucune preuve |
| 6 | Aucune autre normalisation (pas de trim implicite sur les valeurs elles-mêmes ; la clé doit être produite par construction, non par reformatage) |

Exemple : `regle_deterministe|client_id|null|8f14e45f-ea1b-4c2f-9f3e-2a1b3c4d5e6f|N2|L3-N2-1`

Une même clé ne peut apparaître qu'une seule fois. Un second passage automatique avec la même clé ne doit pas ajouter de doublon. Une validation humaine ultérieure utilise `source=humain` et constitue une clé distincte. La stabilité de la clé entre deux traitements est une exigence de test (T-19, T-33).
...
**L.2 Invariant de sécurité — protection absolue des sentinelles humaines.**

Constat factuel : le Lot 3 actuel réécrit certaines métadonnées d'analyse lors d'une réexécution, notamment `analyse.validated_by`, `analyse.validated_at`, `analyse.validation_humaine_requise` et `analyse.provenance.source`. Le Lot 4 **ne doit jamais considérer ces champs comme fiables** si une réexécution du Lot 3 peut les avoir écrasés.

Invariant normatif :

> **Une validation humaine existante constitue une protection absolue contre toute réécriture automatique ultérieure.**

Le Lot 4 doit refuser toute écriture si **l'une quelconque** des conditions suivantes est vraie sur l'état observé :

- `analyse.validated_by != null` ; **ou**
- `analyse.validated_at != null` ; **ou**
- `analyse.provenance.source = "humain"` ; **ou**
- `analyse.statut = "CONFIRMED"`.

Cette protection devra être **garantie de bout en bout** avant toute mise en production du branchement Lot 3 → Lot 4 : une réexécution du Lot 3 ne doit pas pouvoir effacer les sentinelles qui bloquent le Lot 4. Tant que cette garantie n'est pas démontrée (code Lot 3 inclus), le branchement Lot 4 en production est interdit. Le présent document ne modifie pas le code du Lot 3 ; il pose l'exigence.
...
La garde optimiste doit vérifier **systématiquement**, dans toute description ou implémentation :

- l'état de chaque FK attendu (`IS NOT DISTINCT FROM` la valeur observée) ;
- `ai_context` complet attendu ;
- `analyse.statut` attendu ;
- `analyse.validated_by` attendu ;
- `analyse.validated_at` attendu ;
- `analyse.provenance.source` attendu.

Elle doit permettre de détecter au minimum :
...
où chaque FK vaut soit sa valeur actuelle, soit une valeur autorisée par la matrice E. Aucune FK n'est jamais remise à `NULL` par le moteur automatique. Une affectation de 0 ligne vaut rejet et jamais succès. La condition `analyse.provenance.source` fait partie intégrante de la garde : elle est obligatoire partout où la garde de concurrence est décrite ou implémentée, au même titre que M.4.
...
**N.4 Contrôles pré-écriture obligatoires.** Pour chaque FK candidate : format UUID valide, existence de la ligne cible, unicité du candidat, exhaustivité du référentiel, niveau porteur explicite, absence de contradiction, cohérence relationnelle croisée (client du contrat = client du dossier = client retenu), FK actuelle `NULL`, absence de validation humaine antérieure (sentinelles L.2), contexte global non `AMBIGUOUS`, garde d'idempotence, garde optimiste.

**N.5 Traçabilité.** Toute écriture est reconstituable a posteriori depuis `ai_context` seul : niveau de preuve invoqué, identifiants de preuves, horodatage, source déterministe ou humaine, clé de déduplication de trace (K.5).
...
Rappel factuel : le Lot 3 ne produit jamais le statut `A_QUALIFIER` pour le contexte global. Les statuts attendus ci-dessous se limitent aux statuts réellement produits : `DETECTED`, `PROPOSED`, `AMBIGUOUS` (et `CONFIRMED` pour la voie humaine uniquement).
...
| T-24 | Numéro de police fourni sous forme d'UUID inexistant ou cible inexistante | N3 invalide | **Refuser** (existence non vérifiée / confusion identifiant métier-UUID) | aucune | toutes | `AMBIGUOUS` si ambiguïté déclarée par le Lot 3, sinon `DETECTED`/`PROPOSED` non écrit ; jamais `A_QUALIFIER` ; aucune FK |
...
| Critère | État |
|---|---|
| N5 reste N5 ; `personnes_detectees[].email` jamais N2 | ✅ D, E, F, J, O |
| `PROPOSED` n'autorise jamais seul une écriture | ✅ C.2/C.3/D.4/E/N/O |
| Mécanisme de niveau porteur documenté sans modifier le schéma | ✅ C.3 (+ conventions C.3.1/C.3.2) |
| `AMBIGUOUS` global bloque toute écriture automatique | ✅ J.4, L, N, O |
| Garde optimiste contre lost update (incl. `provenance.source`) | ✅ M.4/N.2/O T-31 à T-34 |
| Protection absolue des sentinelles humaines documentée | ✅ L.2, N.4, Q.11 |
...
| Idempotence humaine et automatique définie | ✅ L, K.5 (clé normalisée à 6 composantes) |
| Contradictions bloquantes et antérieures | ✅ J.1/J.4 |
| Atomicité et concurrence définies | ✅ M |
| Plan de recette complet | ✅ 35 scénarios (T-24 aligné sur les statuts réels du Lot 3) |

Statut du présent livrable : **DESIGN CORRIGÉ V1.2 — SOUMIS À AUDIT DG FINAL**. Il ne constitue pas un GO développement.

🛑 **LE DÉVELOPPEMENT DU LOT 4 RESTE GELÉ JUSQU'AU GO DG FINAL DU DESIGN V1.2.**

---

## Q. Inventaire des limites, préconditions et dépendances
...
11. **Dépendance critique — sentinelles humaines vs réexécution du Lot 3.** Le code Lot 3 actuel réécrit `analyse.validated_by`, `analyse.validated_at`, `analyse.validation_humaine_requise` et `analyse.provenance.source` lors d'une nouvelle résolution. L'invariant L.2 (protection absolue d'une validation humaine existante) exige que ces champs ne puissent pas être effacés par une réexécution automatique avant d'être lus par le Lot 4. **Précondition de production** : le branchement Lot 3 → Lot 4 ne peut pas être mis en production tant que cette protection n'est pas garantie de bout en bout (y compris côté code Lot 3). La présente V1.2 documente l'exigence ; elle ne modifie pas le code du Lot 3.

---

🟡 **LOT 4 — DESIGN V1.2 CORRIGÉ — SOUMIS À AUDIT DG FINAL**
