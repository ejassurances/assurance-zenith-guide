...
export async function tariferSanteDossier(
  ...
  if (dos.type_assurance !== 'sante') {
    throw new Error("La tarification Néoliane automatique n'est disponible que sur la branche santé.")
  }
  ...
  if (!/^\d{5}$/.test(zipCode)) {
    throw new Error('Code postal du client absent ou invalide : complétez la fiche client avant de tarifer.')
  }
  ...
  if (profileId === undefined || profileId === null || profileId === '') {
    throw new Error(messageApi('Réponse Néoliane sans identifiant de profil', profil.status, profil.data))
  }
