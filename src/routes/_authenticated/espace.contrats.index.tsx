/**
 * D5 — Portefeuille de contrats (lecture seule) : KPI, filtres et liste.
 */
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[820px] text-sm">
              ...
              <tbody className="divide-y divide-line">
                {filtrees.map((l) => (
                  <tr key={l.contrat_id} className="hover:bg-surface/40">
                    ...
                    <td className="px-3 py-2 text-xs text-ink-muted">
                      ...
                      {l.jours_avant_echeance !== null && (
                        <div>
                          {l.jours_avant_echeance >= 0
                            ? `dans ${l.jours_avant_echeance} j`
                            : `dépassée de ${-l.jours_avant_echeance} j`}
                        </div>
                      )}
                    </td>
                    <td className="px-3 py-2 text-xs text-ink-muted">{fmtDate(l.prochain_suivi_le)}</td>
                    <td className="px-3 py-2 text-right">
                      {l.prime_annuelle !== null ? fmtEuro(l.prime_annuelle) : "—"}
                    </td>
                    <td className="px-3 py-2">
                      <span className={"rounded-full px-2 py-0.5 text-xs font-medium " + BADGE[l.etat]}>
                        {LABEL_ETAT[l.etat]}
                      </span>
                    </td>
                    <td className="px-3 py-2 text-right">
