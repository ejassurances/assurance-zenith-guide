-- Le garde-fou de pipeline (trg_garde_fou_pipeline_documents) ne connaissait
-- que notre propre devoir de conseil (devoirs_conseil.envoye_le). Il bloquait
-- donc le passage à contrat_valide même quand le client a signé le devoir de
-- conseil DE LA COMPAGNIE, avec preuve déposée au dossier (documents,
-- type_document = 'devoir_conseil_compagnie') — cas déjà géré côté
-- application (souscription-prerequis.ts) mais pas ici, au niveau base.
-- Même principe : la preuve documentaire suffit, jamais une dérogation
-- sans justificatif.
...
begin
  ...
  if _idx > _idx_dc then
    select exists (
      ...
      where d.dossier_id = new.id
        and d.type_document = 'devoir_conseil_compagnie'
        and d.archive_le is null
    ) into _dc_ok;
    if not _dc_ok then
      raise exception 'Étape « % » impossible : aucun devoir de conseil réellement envoyé pour ce projet (le nôtre, ou celui de la compagnie déposé au dossier).', new.statut
        using errcode = 'check_violation';
    end if;
  end if;
