-- Fonctions internes (déclencheurs) : non appelables depuis l'API.
REVOKE EXECUTE ON FUNCTION public.trg_devis_une_tete_par_devis() FROM anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.trg_recueil_emprunteur_sans_sante() FROM anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.trg_bloquer_suppression_dda() FROM anon, authenticated;
REVOKE EXECUTE ON FUNCTION public.nb_assures_emprunteur(uuid) FROM anon;

-- Clarification : la liste des clés conservées pour chaque assuré emprunteur
-- (aucune donnée de santé) est renommée pour lever toute ambiguïté.
...
DECLARE
  ...
  -- Seules ces clés sont conservées pour chaque assuré : aucune donnée médicale.
